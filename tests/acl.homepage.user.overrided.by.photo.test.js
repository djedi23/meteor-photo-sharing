...
var url=baseURL+'/';
testId="acl_homepage_user_overrided_by_photo";


casper.test.begin("Test acl user homepage overrided by a photo", function suite(test) {
  ...
  casper.thenClick('input[data-key="inHomepage"].permission', function(){
    casper.echo("photo explicitement partagée","INFO");
    /*
     * PUBLIC: acl share
     * user:  true  true
     * album: true  undef
    ...
    assertMongo.call(test, 'db.photos.findOne({_id:"'+photoAlbumId+'"})', 'A photo.share === true' ,function(value){return value.share.inHomepage===true && value.share.isPublic===true; });
  ...
  casper.thenClick('input[data-key="isPublic"].permission', function(){
    casper.echo("photo explicitement non partagée","INFO");
    /*
     * PUBLIC: acl share
     * user:  true  true
     * album: true  undef
    ...
    assertMongo.call(test, 'db.acl.findOne({collection:"users", id:"'+userId+'", key:"share.inHomepage"}).albums', 'user inHomepage contient 1 album' ,function(value){ return value.indexOf(albumId) !== -1; });
    assertMongo.call(test, 'db.photos.findOne({_id:"'+photoId+'"})', 'B photo.share === undef' ,function(value){return value.share === undefined; });
    assertMongo.call(test, 'db.photos.findOne({_id:"'+photoAlbumId+'"})', 'A photo.share === false' ,function(value){return value.share.inHomepage===false && value.share.isPublic===false; });
    assertMongo.call(test, 'db.albums.findOne({_id:"'+albumId+'"})', 'albums.share === true' ,function(value){return value.share === undefined; });
    assertMongo.call(test, 'db.users.findOne({_id:"'+userId+'"})', 'users.share === true' ,function(value){return value.share.inHomepage===true && value.share.isPublic===true; });
  });
  logout();
